...
class OTPBloc{
  ...
  async {
    ...
    AuthCredential phoneAuthCredential = PhoneAuthProvider.credential(verificationId: authVerificationId, smsCode: smsCode);

    // Sign the user in (or link) with the credential
    await auth.signInWithCredential(phoneAuthCredential).catchError((error) {
      Loader.hideLoader();
      AppDialog.showErrorDialog(context, "", "Something has gone wrong, please try later", 0, onRetry: () {  });
    }).then((value) {
      Loader.hideLoader();
      if(value!=null && value.user!=null)
      {
        ...
        AppDialog.showErrorDialog(context, "", "Invalid code/invalid authentication", 0, onRetry: () {  });
      }
    }
    );
  }
